...
class BBPSpec extends Suites {
  ...
  private val valid = BuildInfo.data.foldLeft(List.empty[Uri]) {
    case (acc, e: String) => s"""${BuildInfo.base}${e.substring(0, e.lastIndexOf(".json"))}""" :: acc
  }
